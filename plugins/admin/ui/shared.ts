...
export function pickerButton(id: string, label: string, open: () => void) {
  return html`<button type="button" class="picker-trigger" id=${id} aria-haspopup="dialog" @click=${open}>
    <span>${label}</span
    ><svg
      width="16"
      height="16"
