...
export function loadCredentials(rows: Data[], directory: Data[], channels: Data[], scopes: Data[], scope: string) {
  Object.assign(credentialState, { rows, directory, channels, scopes, scope });
        ...
        (c) => {
          ...
          >
            <div class="credential-main">
              ...
              <div class="hint">
                ${target + " · " + (c.usageTruncated ? "at least " : "") + plural(c.usageCount || 0, "successful use") + (c.usageCount ? " since " + config.formatTime(c.usageSince) : " in retained broker history")}
              </div>
...
function template() {
  ...
  >
    ...
    <div class="body">
      <div id="sc-list" class=${credentialState.rows.length ? "" : "hint"}>${credentialRows()}</div>
    </div>
    <div
    ...
    <div class="foot">
      ...
      ><button type="button" id="sc-add" class=${credentialState.open ? "hidden" : ""}>+ Add credential</button
      ><button
