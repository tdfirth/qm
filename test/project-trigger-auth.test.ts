...
test("Project trigger access follows current membership while Slack-group owner access is unchanged", async (t) => {
  ...
  const server = serveApp(built.app, { signingSecret: SECRET, scheduler: built.scheduler }, "127.0.0.1");
  ...
  assert.equal(
    (await fetch(`${server.base}/v1/soul`, { headers: { "x-agent-capability": staleOwnerToken } })).status,
    403,
  );
  assert.equal(
    (await fetch(`${server.base}/v1/soul`, { headers: { "x-agent-capability": projectToken } })).status,
    200,
  );
  assert.equal(
    (
      await fetch(`${server.base}/v1/crons`, {
        method: "POST",
  ...
  assert.equal(
    (
      await fetch(`${server.base}/v1/soul`, {
        headers: { "x-agent-capability": await token("member", `personal:member`) },
      })
    ).status,
    200,
  );
  assert.equal((await fetch(`${server.base}/v1/soul`, { headers: { "x-agent-capability": slackToken } })).status, 200);
  ...
  const cronList = await fetch(`${server.base}${cronListPath}`, { headers: signed("GET", cronListPath) });
  ...
  for (const suffix of [
    ...
    assert.equal((await fetch(`${server.base}${suffix}`, { headers: signed("GET", suffix) })).status, 404);
  }
  const projectPatchPath = `/v1/crons/${projectCron.id}?principalId=member`;
  const projectPatchBody = JSON.stringify({ title: "hijack" });
  assert.equal(
    (
      await fetch(`${server.base}${projectPatchPath}`, {
        method: "PATCH",
  ...
  assert.equal((await fetch(`${server.base}${slackGetPath}`, { headers: signed("GET", slackGetPath) })).status, 200);
  const publicGetPath = `/v1/crons/${publicCron.id}?principalId=member`;
  assert.equal((await fetch(`${server.base}${publicGetPath}`, { headers: signed("GET", publicGetPath) })).status, 200);
  const publicRunsPath = `/v1/crons/${publicCron.id}/runs?principalId=member`;
  assert.equal(
    (await fetch(`${server.base}${publicRunsPath}`, { headers: signed("GET", publicRunsPath) })).status,
    404,
  );

  const webhookListPath = "/v1/webhooks?viewer=member";
  const webhookList = await fetch(`${server.base}${webhookListPath}`, { headers: signed("GET", webhookListPath) });
  ...
  assert.equal(
    (
      await fetch(`${server.base}${projectDisablePath}`, {
        method: "POST",
  ...
  assert.equal(
    (
      await fetch(`${server.base}${slackDisablePath}`, {
        method: "POST",
