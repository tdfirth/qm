...
import { serve, serveApp, stubHttp } from "./support/api.ts";
...
test("/d/ proxy attaches the endpoint's proxyHeaders — the internal path authenticates to a token-gated deployment", async () => {
  ...
  const server = serveApp(
    appWith({ host: "127.0.0.1", port: upstream.port, proxyHeaders: { cookie: "dpl_access=tok" } }),
  );
  try {
...
test("/d/ proxy multiplexes concurrent requests over one HTTP/2 session", async () => {
  ...
  upstream.server.on("session", (session) => {
    sessionCount++;
  ...
  try {
    ...
    const events = await fetch(`${server.base}/d/some-id/events`, { signal: eventsAbort.signal });
    const eventsReader = events.body!.getReader();
    assert.match(Buffer.from((await eventsReader.read()).value!).toString(), /data: ready/);
    const responses = await Promise.all(
      Array.from({ length: 16 }, (_, i) => fetch(`${server.base}/d/some-id/request-${i}`)),
    );
    assert.deepEqual(
    ...
    const afterAbort = await fetch(`${server.base}/d/some-id/after-abort`);
...
test("/d/ proxy retires a GOAWAY session even while an SSE stream is open", async () => {
  ...
  const server = serveApp(appWith({ host: "127.0.0.1", port: upstream.port, httpVersion: "2" }));

  try {
    const events = await fetch(`${server.base}/d/some-id/events`);
    const reader = events.body!.getReader();
    assert.match(Buffer.from((await reader.read()).value!).toString(), /data: ready/);
    const slow = fetch(`${server.base}/d/some-id/slow`);
    await slowStart;
    sessions[0]!.goaway();
    await new Promise((resolve) => setTimeout(resolve, 20));

    const replacement = await fetch(`${server.base}/d/some-id/after-goaway`);
...
test("/d/ proxy retries a GOAWAY-refused GET once but never replays a POST body", async () => {
  ...
  let postHits = 0;
  let deadHits = 0;
  const upstream = serve(createHttp2Server());
  upstream.server.on("session", (session) => {
    sessions.add(session);
    session.on("close", () => sessions.delete(session));
  });
  upstream.server.on("stream", (stream, headers) => {
  ...
  try {
    assert.equal((await fetch(`${server.base}/d/some-id/warm`)).status, 200);
    const retried = await fetch(`${server.base}/d/some-id/retry-get`);
    assert.equal(retried.status, 200);
    assert.equal(await retried.text(), "ok");
    assert.equal(getHits, 2);

    const streamed = await fetch(`${server.base}/d/some-id/retry-stream`);
    assert.equal(
      await streamed.text(),
      "first\nsecond\n",
    ...
    const mutation = await fetch(`${server.base}/d/some-id/no-retry-post`, { method: "POST", body: "must-not-replay" });
    assert.equal(mutation.status, 502);
    assert.equal(postHits, 1);

    const recovered = await fetch(`${server.base}/d/some-id/dead-session`);
...
test("/d/ proxy keeps the shared HTTP/2 session when one request has invalid headers", async () => {
  ...
  try {
    ...
    assert.match(Buffer.from((await reader.read()).value!).toString(), /data: ready/);
    invalidHeaders = true;
    assert.equal((await fetch(`${server.base}/d/some-id/invalid`)).status, 502);
    invalidHeaders = false;
    assert.equal((await fetch(`${server.base}/d/some-id/healthy`)).status, 200);
...
test("/d/ proxy never retries after an HTTP/2 timeout has committed a 504", async () => {
  ...
  upstream.server.on("stream", (stream) => {
    stream.on("error", () => undefined);
    hits++;
  ...
  try {
    const response = await fetch(`${server.base}/d/some-id/timeout`);
  ...
  try {
    const outcome = await new Promise<{ body: string; aborted: boolean }>((resolve, reject) => {
      const request = httpRequest(
        { host: "localhost", port: server.port, path: "/d/some-id/truncated" },
        (response) => {
          const chunks: Buffer[] = [];
  ...
  const app = {
    ...
    reachDeployment: async (_id: string, _principal: string, opts?: { bypassAcl?: boolean }) => {
      ...
      return opts?.bypassAcl
        ? { status: "ok", endpoint: { host: "127.0.0.1", port: upstream.port } }
        : { status: "denied" };
    },
  } as unknown as App;
...
test("/d/ proxy forwards the client's content-type and body with an explicit content-length — never chunked (the MicroVM ingress rejects chunked uploads)", async () => {
  ...
  try {
    ...
    const res = await fetch(`${server.base}/d/some-id/api/query`, {
      method: "POST",
...
test("/d/ proxy forwards client cookies but scrubs the dpl_access gate token, and endpoint proxyHeaders still win", async () => {
  ...
  const server = serveApp(
    appWith({ host: "127.0.0.1", port: upstream.port, proxyHeaders: { "X-aws-proxy-auth": "vm-token" } }),
  );
  try {
    const res = await fetch(`${server.base}/d/some-id/`, {
      headers: {
...
test("/d/ proxy strips fixed and Connection-named hop-by-hop response headers", async () => {
  ...
  try {
    const headers = await new Promise<Record<string, string | string[] | undefined>>((resolve, reject) => {
      const rq = httpRequest({ host: "localhost", port: server.port, path: "/d/some-id/" }, (rs) => {
        rs.resume();
