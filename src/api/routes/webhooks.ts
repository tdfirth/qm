...
import { PayloadTooLargeError, badRequest, notFound, readRawBody, sendJson, unauthorized } from "../http.ts";
...
async function incomingWebhook(ctx: BaseCtx): Promise<void> {
  ...
  if (out.status === 401) return unauthorized(res, "signature verification failed");
  return notFound(res);
}
...
async function createWebhook(ctx: ApiCtx): Promise<void> {
  ...
  if (capability) {
    ...
    if (!ver || !isWebhookVerification(ver) || typeof b.action !== "string") {
      return badRequest(res, "a supported signature scheme and non-empty signing secret are required");
    }
    if (b.filters !== undefined && !isWebhookFilters(b.filters)) {
      return badRequest(res, "every filter requires a non-empty path and at least one non-empty value");
    }
  ...
  if (!isCreateWebhook(body)) return badRequest(res, "expected a CreateWebhookInput");
  try {
