...
test("init scaffolds a loadable config, generated local secrets, and a valid sandbox/ layer", (t) => {
  const base = tempDir(t, "qm-init-");
  ...
  for (const line of ['# Needed when env.core.HARNESS is "codex" or modelProvider is "openai".', "# OPENAI_API_KEY="]) {
...
test("init --target fly scaffolds the full hosted topology and both Slack apps", (t) => {
  const dir = tempDir(t, "qm-init-fly-");
...
test("init --email-transport smtp scaffolds smtp keys only and a matching config", (t) => {
  const dir = tempDir(t, "qm-init-smtp-");
...
test("init keeps stable qm Slack branding for long org ids", (t) => {
  for (const org of ["a".repeat(34), `${"a".repeat(28)}-bcd`]) {
    const dir = tempDir(t, "qm-init-");
...
test("init scaffolds no sandbox block — sandboxes boot stock platform images", (t) => {
  const dir = tempDir(t, "qm-init-");
  quiet(() => runInit({ dir, org: "globex" }));
  assert.equal(loadConfigInDir(dir).config.sandbox, undefined);
});

test("init --target aws scaffolds the full hosted topology, Terraform, and the optional Slack bot manifest", (t) => {
  const dir = tempDir(t, "qm-init-");
...
test("init keeps long next-step commands separate from their explanations", (t) => {
  const dir = tempDir(t, "qm-init-");
...
test("init --target aws derives stable valid AWS names for a maximum-length org id", (t) => {
  const org = `a${"b".repeat(62)}`;
  const first = tempDir(t, "qm-init-long-");
  const second = tempDir(t, "qm-init-long-");
...
test("init refuses to clobber an existing config, and leaves present scaffold files untouched", (t) => {
  const dir = tempDir(t, "qm-init-");
  quiet(() => runInit({ dir, org: "acme" }));
  assert.throws(() => quiet(() => runInit({ dir, org: "acme" })), /already exists/);
});

test("init makes .env the final matching gitignore rule before generating signing keys", (t) => {
  const dir = tempDir(t, "qm-init-existing-ignore-");
...
test("init refuses to generate keys into an absent but Git-tracked .env", (t) => {
  const dir = tempDir(t, "qm-init-tracked-env-");
...
test("init refuses a present tracked .env before writing the scaffold", (t) => {
  const dir = tempDir(t, "qm-init-present-tracked-env-");
  execFileSync("git", ["init"], { cwd: dir, stdio: "ignore" });
  writeFileSync(join(dir, ".env"), "previous=value\n");
  execFileSync("git", ["add", ".env"], { cwd: dir, stdio: "ignore" });
  assert.throws(() => quiet(() => runInit({ dir, org: "acme" })), /tracked by Git/);
...
test("init merges every AWS secret and generated-state ignore into an existing file", (t) => {
  const dir = tempDir(t, "qm-init-aws-ignore-");
...
test("init preflights package.json and completes an install-first package manifest", (t) => {
  const invalid = tempDir(t, "qm-init-invalid-package-");
  const existing = tempDir(t, "qm-init-existing-package-");
  ...
  assert.ok(!existsSync(join(invalid, CONFIG_FILENAME)), "invalid package metadata cannot strand a partial deployment");
...
test("init preserves an installed local package artifact", (t) => {
  const dir = tempDir(t, "qm-init-local-package-");
  writeFileSync(
...
test("init --target aws vendors a contract-valid Terraform deployment", (t) => {
  const dir = tempDir(t, "qm-init-aws-");
...
test("the scaffold is an npm-backed deployment repository with no CI coupling and valid Slack YAML", (t) => {
  const dir = tempDir(t, "qm-init-");
  quiet(() => runInit({ dir, org: "acme" }));
