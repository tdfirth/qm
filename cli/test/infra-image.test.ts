...
function harness(t: TestContext, env: Record<string, string>): Harness {
  const dir = tempDir(t, "qm-infra-image-");
  ...
  setEnv(t, {
    ...
    AWS_BIN: fakeAws(dir),
    FAKE_AWS_LOG: log,
    FAKE_AWS_STATE: join(dir, "state"),
    ...env,
  });
  return { dir, configPath, log, calls: () => readFileSync(log, "utf8") };
}

test("infra delete-image accepts an empty successful S3 version response", async (t) => {
  const { calls } = harness(t, { FAKE_MODE: "deleted", FAKE_EMPTY_OBJECT_VERSIONS: "1" });
...
for (const mode of ["create", "update"] as const) {
  test(`infra build-image ${mode}s the image and records immutable runtime coordinates`, async (t) => {
    const { configPath, calls } = harness(t, { FAKE_MODE: mode });
    ...
    assert.match(
      logged,
      new RegExp(`lambda-microvms get-microvm-image-version --image-identifier .* --image-version ${expectedVersion}`),
    );
...
test("infra build-image checks the AWS account before uploading", async (t) => {
  const { configPath, calls } = harness(t, { FAKE_ACCOUNT: "999999999999" });
...
test("infra build-image uses a fresh idempotency token for each explicit rebuild", async (t) => {
  const { configPath, calls } = harness(t, { FAKE_MODE: "create" });
...
test("infra build-image reports failure from the exact version returned by update", async (t) => {
  const { configPath, calls } = harness(t, { FAKE_MODE: "update", FAKE_VERSION_FAIL: "1" });
...
for (const scenario of ["inflight", "conflict"] as const) {
  test(`infra build-image waits out a ${scenario} build and polls only its own returned version`, async (t) => {
    const env: Record<string, string> =
      scenario === "inflight" ? { FAKE_MODE: "inflight" } : { FAKE_MODE: "update", FAKE_CONFLICT: "1" };
    const { configPath, calls } = harness(t, env);
    const result = await buildAwsMicrovmImage(config(), configPath, { intervalMs: 0 });
...
for (const contender of ["build-image", "delete-image"] as const) {
  test(`infra image lifecycle lease serializes build-image against ${contender}`, async (t) => {
    const { configPath, calls } = harness(t, { FAKE_MODE: "create" });
    let entered!: () => void;
    let resume!: () => void;
...
for (const operation of ["build", "delete"] as const) {
  test(`infra ${operation}-image refuses an image with an unexpected ARN before mutating anything`, async (t) => {
    const { configPath, calls } = harness(t, {
      FAKE_MODE: "update",
      FAKE_IMAGE_ARN: "arn:aws:lambda:us-east-1:999999999999:microvm-image:acme-microvm",
    });
    const run =
      operation === "build"
        ? buildAwsMicrovmImage(config(), configPath, { intervalMs: 0 })
...
test("infra build-image refuses an operation response naming an unexpected image", async (t) => {
  const { configPath, calls } = harness(t, { FAKE_MODE: "create", FAKE_OP_NAME: "other-image" });
  await assert.rejects(buildAwsMicrovmImage(config(), configPath, { intervalMs: 0 }), /unexpected MicroVM image name/);
  assert.doesNotMatch(calls(), /get-microvm-image-version/);
});

for (const operation of ["build", "delete"] as const) {
  test(`infra ${operation}-image refuses a same-name image owned by another deployment`, async (t) => {
    const { configPath, calls } = harness(t, { FAKE_MODE: "update", FAKE_DEPLOYMENT_TAG: "other" });
    const run =
      operation === "build"
        ? buildAwsMicrovmImage(config(), configPath, { intervalMs: 0 })
        : deleteAwsMicrovmImage(config(), { intervalMs: 0 });
...
for (const mode of ["update", "deleted"] as const) {
  test(`infra delete-image is idempotent for ${mode === "deleted" ? "an already deleted" : "an active"} image and removes versioned artifacts`, async (t) => {
    const { calls } = harness(t, { FAKE_MODE: mode });
    await deleteAwsMicrovmImage(config(), { intervalMs: 0 });
    const logged = calls();
